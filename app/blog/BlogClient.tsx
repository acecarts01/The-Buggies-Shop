  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#121417] text-[#ffffff]">
      ...
      <div className="bg-[#0A0B0D] border-b border-[#1F2226] px-4 py-2 text-xs">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-[#A8A29E]">
          <Link href="/" className="hover:text-[#E2A17A] transition-colors">
            Home
          </Link>
          <span>/</span>
          <span className="text-[#E2A17A] font-medium">Golf Buggy Blog &amp; Guides</span>
        </div>
      </div>

      <main id="main" className="flex-1">
        {/* Hero Section */}
        <section className="relative py-14 sm:py-18 px-4 border-b border-[#2B2F34] bg-gradient-to-b from-[#16181B] to-[#121417]">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="max-w-3xl">
                <FadeUpText delay={0.05} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#1A1D21] border border-[#E2A17A]/40 text-[#E2A17A] text-xs font-semibold uppercase tracking-wider mb-4 metal-brushed-dark">
                ...
                />
                <StaggeredParagraph delay={0.15} className="mt-4 text-base sm:text-lg text-[#A8A29E] leading-relaxed">
              ...
              <AnimatedCard delay={0.2} className="bg-[#1A1D21] border border-[#2B2F34] rounded-xl p-4 sm:p-5 shrink-0 max-w-sm metal-brushed-dark">
                <div className="flex items-center gap-2 text-xs font-semibold text-[#E2A17A] uppercase tracking-wider mb-1">
                  <ShieldCheck className="w-4 h-4 text-[#E2A17A]" />
                  <span>Workshop Enquiries</span>
                </div>
                <div className="text-xs text-[#D6D3D1] mb-2">
                  Need tailored advice for your estate, farm, or golf club? Speak directly with our Yatala engineers.
                </div>
                <a
                  href={`tel:${CONTACT.phone}`}
                  className="inline-flex items-center gap-2 text-sm font-bold text-[#ffffff] hover:text-[#E2A17A] transition-colors"
                >
            ...
            <div className="mt-10 pt-6 border-t border-[#1F2226] flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
              {/* Category Pills */}
              <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-none" role="tablist">
                <span className="text-xs font-semibold text-[#A8A29E] uppercase tracking-wider flex items-center gap-1 shrink-0 mr-1">
                  <Filter className="w-3.5 h-3.5 text-[#E2A17A]" />
                  <span>Category:</span>
                </span>
                {categories.map((cat) => (
                  <button
              ...
              <div className="relative min-w-[240px] sm:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 text-xs bg-[#0A0B0D] border border-[#2B2F34] rounded-lg text-[#ffffff] placeholder-[#78716C] focus:outline-none focus:border-[#E2A17A] transition-colors"
                />
              </div>
            </div>
          </div>
        ...
        {selectedCategory === 'All' && !searchQuery && featuredPost && (
          <section className="py-10 px-4 border-b border-[#2B2F34] bg-[#0A0B0D]/60">
            <div className="max-w-7xl mx-auto">
              <FadeUpText delay={0.05} className="text-xs font-bold uppercase tracking-wider text-[#E2A17A] mb-3 flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" />
                <span>Featured Technical Article</span>
              </FadeUpText>

              <AnimatedCard delay={0.1} className="bg-[#1A1D21] border border-[#2B2F34] rounded-2xl p-6 sm:p-8 lg:p-10 shadow-sm hover:border-[#E2A17A]/50 transition-all metal-brushed-dark">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                  <div className="lg:col-span-8 space-y-4">
                    <div className="flex flex-wrap items-center gap-3 text-xs text-[#A8A29E]">
                      <span className="px-2.5 py-1 rounded bg-[#121417] border border-[#2B2F34] text-[#E2A17A] font-semibold">
                      ...
                      <span className="flex items-center gap-1 text-[#E7E5E4]">
                        <User className="w-3.5 h-3.5 text-[#E2A17A]" />
                        {featuredPost.author.name}
                      </span>
                    </div>

                    <h2 className="text-2xl sm:text-3xl font-serif font-bold text-[#ffffff] leading-snug tracking-tight">
                      <Link
                        href={`/blog/${featuredPost.slug}/`}
                        className="hover:text-[#E2A17A] transition-colors"
                      >
                        {featuredPost.title}
                      </Link>
                    </h2>

                    <StaggeredParagraph delay={0.1} className="text-sm sm:text-base text-[#A8A29E] leading-relaxed">
                      {featuredPost.excerpt}
                    </StaggeredParagraph>

                    {/* Key takeaways snippet */}
                    <div className="p-4 bg-[#121417] border border-[#2B2F34] rounded-xl text-xs space-y-2">
                      <div className="font-bold text-[#E2A17A] uppercase tracking-wider text-[11px]">
                        Core Technical Takeaways:
                      </div>
                      <ul className="space-y-1 text-[#D6D3D1]">
                        {featuredPost.keyTakeaways.slice(0, 2).map((takeaway, idx) => (
                          <li key={idx} className="flex items-start gap-2">
                            <span className="text-[#E2A17A] font-bold shrink-0">•</span>
                            <span>{takeaway}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="pt-2 flex flex-wrap items-center gap-4">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 text-xs text-[#AEB4B8] hover:text-[#E2A17A] font-medium transition-colors"
                      >
                  ...
                  <div className="lg:col-span-4 bg-[#121417] border border-[#2B2F34] rounded-xl p-5 space-y-3">
                    ...
                    <StaggeredParagraph delay={0.15} className="text-xs text-[#A8A29E] leading-relaxed">
                    ...
                    <div className="pt-2 space-y-1.5 text-[11px] text-[#D6D3D1]">
                      <div className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#E2A17A]" />
        ...
        <section className="py-12 px-4 max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            {selectedCategory !== 'All' && (
              <button
                ...
                className="text-xs text-[#E2A17A] hover:underline font-semibold"
              >
                Reset to All Guides
              </button>
            )}
          ...
          {filteredPosts.length === 0 ? (
            <div className="bg-[#1A1D21] border border-[#2B2F34] rounded-xl p-12 text-center metal-brushed-dark">
              <BookOpen className="w-8 h-8 text-[#E2A17A] mx-auto mb-3" />
              ...
              <button
                ...
                className="mt-4 px-4 py-2 rounded-lg bg-[#E2A17A] text-[#121417] font-bold text-xs hover:bg-[#EFC7A6] transition-colors"
              >
                Clear Filters
              </button>
            </div>
                  ...
                  <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center justify-between gap-2 text-[11px] mb-3">
                        <span className="px-2.5 py-0.5 rounded bg-[#0A0B0D] border border-[#2B2F34] text-[#E2A17A] font-semibold">
                          {post.category}
                        </span>
                        <span className="flex items-center gap-1 text-[#A8A29E]">
                          <Clock className="w-3 h-3 text-[#AEB4B8]" />
                          {post.readTime}
                        </span>
                      </div>

                      <h3 className="text-lg font-serif font-bold text-[#ffffff] group-hover:text-[#E2A17A] transition-colors leading-snug line-clamp-2">
                        <Link href={`/blog/${post.slug}/`}>
                          {post.title}
                        </Link>
                      </h3>

                      <StaggeredParagraph delay={0.05} className="mt-2.5 text-xs text-[#A8A29E] leading-relaxed line-clamp-3">
                        {post.excerpt}
                      </StaggeredParagraph>
                      ...
                      <div className="mt-4 flex flex-wrap gap-1.5">
                        {post.tags.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded bg-[#121417] border border-[#1F2226] text-[#D6D3D1]"
                          >
                            <Tag className="w-2.5 h-2.5 text-[#AEB4B8]" />
                            {tag}
                          </span>
                        ))}
                      </div>
        ...
        <section className="py-12 px-4 bg-[#0A0B0D] border-t border-[#2B2F34]">
          <div className="max-w-4xl mx-auto text-center space-y-4">
            ...
            <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
              <Link
                href="/contact/"
                className="px-5 py-2.5 rounded-lg bg-gradient-to-r from-[#C86D51] to-[#E2A17A] text-[#121417] font-bold text-xs shadow hover:from-[#E2A17A] hover:to-[#EFC7A6] transition-all"
              >
